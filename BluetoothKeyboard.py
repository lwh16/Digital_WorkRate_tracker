...
def keys_per_time():
    ...
    while True:
        if (time.time() > timer + 60):
            ...
            #file1.close()
            send_details(str(key_count[0]), str(mouse_travel[0]), str(scroll_travel[0]))
            timer = time.time()
            key_count = [0]
            mouse_travel = [0]
            scroll_travel = [0]
...
def send_details(mouse_travel, key_count, scroll_travel):
    flag = True
    while flag:
        try:
            ...
            print("Bluetooth not connecting...")
            time.sleep(1)
            flag = True
